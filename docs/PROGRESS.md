...
- 当前进展：
  ...
  - 后台配置中心新增“榜单配置”面板：可视化列出所有榜单，支持新建/编辑（名称、描述、filters JSON、排序字段、显示数量、自动刷新分钟数、是否公开等）并一键触发手动刷新；前端与新字段打通并提供 JSON 输入提示。
  - 新增榜单独立详情页 `/leaderboards/:id`，支持分享/打印、查看过滤规则与完整排名。榜单列表页可跳转至独立页以便传播引用。
  - 标签管理后台上线：可视化维护字典、规则 JSON、颜色/类型/父子关系，支持新增、编辑、删除；前端钱包列表可按标签筛选，导入时能批量打标。
  - AI 配置后台上线：支持启用开关、自定义 Provider/Key/模型/温度/速率限制及多段 Prompt，后端提供 `/ai/config` 接口；AI 分析状态可在运营页查看，关闭开关后详情页提示“未启用”。
  - AI 日志：调用 AI 分析时自动记录 `ai_logs`（钱包、模型、状态、响应/错误、耗时），并在 `operations` API 暴露 `/ai/logs` 供后台排查；同时支持根据 AI 输出自动映射标签。
  - AI 叙事 & 资金概览：新增 `/wallets/{addr}/ai` GET/POST 接口，AI 结果包含 narrative + metrics；前端详情页展示 AI 评分/收益/回撤卡片与多段分析文本。资金流统计（累计存入/取出/净流入）也在详情页可视化，辅助评估资金安全。
- 下一步：
  - 将 AI 输出与榜单/标签联动（如 AI 标签自动打标、榜单筛选 AI 结果）。
  - 丰富标签统计，并在前台提供更多基于标签/AI 的筛选入口。
